...
function startBirthday(name, age, photo) {
  currentName = name;

  document.getElementById("name").textContent = name;
  document.getElementById("greetingText").textContent = `Selamat ulang tahun, ${name}! 🎉`;
  document.getElementById("imagePath").src = photo;
  document.getElementById("wishName").textContent = name;

  if (age) {
    document.getElementById("ageDisplay").textContent = "🎂 " + age + " Tahun";
  }

  Swal.fire({
    title: "Apakah kamu ingin memutar musik latar?",
    icon: "question",
...
const animationTimeline = () => {
  ...
  tl.to(".container", 0.6, { visibility: "visible" })
    ...
    .call(() => {
      gsap.to(".wa-section", { opacity: 1, pointerEvents: "auto", duration: 0.8, ease: "power3.out" });
    });

  const replyBtn = document.getElementById("replay");
  replyBtn.addEventListener("click", () => {
    gsap.set(".wa-section", { opacity: 0, pointerEvents: "none" });
    tl.restart();
  });
};
